    ...
    <PageShell title="How was your ride?" subtitle="Your feedback keeps chauffeurs at their best.">
      <form onSubmit={(e) => {
        ...
        <label className="mt-6 block">
          ...
          <textarea value={review} onChange={e => setReview(e.target.value)} rows={5} maxLength={1000}
            placeholder="What went well? What could be better?"
            className="mt-1 w-full rounded-xl border border-border bg-transparent px-3 py-2.5 resize-none" />
        </label>
        <button className="mt-6 w-full rounded-2xl gold-gradient px-5 py-3 text-sm font-semibold text-background">Submit review</button>
      </form>
    </PageShell>
  );
}
