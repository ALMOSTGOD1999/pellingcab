...
import { CalendarClock, Mail, MapPin, Phone, User, Clock as ClockIcon, Navigation } from "lucide-react";
      ...
      <form onSubmit={submit} className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
        <div className="glass rounded-3xl p-5 sm:p-7 space-y-5">
          <div className="flex gap-2">
            {(["half", "full"] as const).map(k => (
              <button
                key={k} type="button" onClick={() => setForm({ kind: k })}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Pickup" icon={<MapPin className="h-4 w-4" />} error={errors.pickup}>
              <TextInput placeholder="Hotel Taj, Delhi" value={form.pickup} onChange={e => setForm({ pickup: e.target.value })} />
            </Field>
            <Field label="Destination" icon={<Navigation className="h-4 w-4" />} error={errors.destination}>
              <TextInput placeholder="Agra Fort" value={form.destination} onChange={e => setForm({ destination: e.target.value })} />
            </Field>
            <Field label="Pickup date" icon={<CalendarClock className="h-4 w-4" />}>
              <TextInput type="date" value={form.date} onChange={e => setForm({ date: e.target.value })} />
            </Field>
            <Field label="Pickup time" icon={<ClockIcon className="h-4 w-4" />}>
              <TextInput type="time" value={form.time} onChange={e => setForm({ time: e.target.value })} />
            </Field>
          </div>

          <button type="submit" className="w-full rounded-2xl gold-gradient px-5 py-4 text-sm font-semibold text-background transition hover:brightness-110 active:scale-[0.99]">
        ...
        <aside className="glass rounded-3xl p-6 h-fit">
          ...
          <div className="mt-6 rounded-2xl border border-border bg-background/40 p-4 text-xs text-muted-foreground">
            <strong className="text-primary">Flexible cancellation.</strong> Cancel free up to 2 hours before pickup — full refund to your original payment method.
          </div>
        </aside>
      </form>
    </PageShell>
