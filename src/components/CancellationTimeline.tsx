      ...
      {cancellationSteps.map((s, i) => {
        ...
        return (
          <li key={s.key} className="relative flex items-start gap-4">
            <span
              className={`relative z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full transition
            ...
            <div className="min-w-0 pt-0.5">
              <p
                className={`text-sm ${done ? "text-foreground" : "text-muted-foreground"} ${current ? "font-semibold" : ""}`}
              >
                {s.label}
              </p>
