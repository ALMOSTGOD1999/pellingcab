  ...
  return (
    <article className="group glass rounded-3xl overflow-hidden transition hover:-translate-y-1 hover:shadow-glow">
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        <span className="absolute top-3 left-3 glass rounded-full px-3 py-1 text-[11px] uppercase tracking-widest text-primary">
          {v.category}
        </span>
      ...
      <div className="p-5">
        ...
        <ul className="mt-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          ...
          <Chip>
            <Briefcase className="h-3.5 w-3.5" /> {v.luggage} bags
          </Chip>
          {v.ac && (
            <Chip>
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <li className="inline-flex items-center gap-1 rounded-full border border-border bg-card/60 px-2.5 py-1">
      {children}
    </li>
  );
}
