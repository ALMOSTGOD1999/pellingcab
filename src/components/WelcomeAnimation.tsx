  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-70"
        style={{ background: "radial-gradient(60% 40% at 50% 55%, color-mix(in oklab, var(--gold) 22%, transparent), transparent 70%)" }} />
      {/* logo */}
      <div className="relative z-10 text-center">
        <div className="inline-flex items-center gap-3 opacity-0 animate-logo-rise" style={{ animationDelay: "0.2s" }}>
          <span className="grid h-11 w-11 place-items-center rounded-full gold-gradient text-background font-black text-lg shadow-glow">P</span>
          <span className="font-display text-4xl sm:text-5xl gold-text">PellingCab</span>
        </div>
        <p className="mt-3 text-sm tracking-[0.35em] uppercase text-muted-foreground opacity-0 animate-logo-rise"
           style={{ animationDelay: "0.6s" }}>Chauffeured Luxury</p>
      </div>

      {/* road line */}
      <div className="pointer-events-none absolute left-0 right-0 bottom-[36%] h-[2px]"
        style={{
          backgroundImage: "linear-gradient(to right, transparent, color-mix(in oklab, var(--gold) 60%, transparent), transparent)",
        }}
      />
      <div className="pointer-events-none absolute left-0 right-0 bottom-[34%] h-[6px] opacity-70 animate-road"
        style={{
          backgroundImage: "repeating-linear-gradient(to right, color-mix(in oklab, var(--gold) 70%, transparent) 0 24px, transparent 24px 56px)",
        }}
      />
...
function CarSVG() {
  return (
    <svg width="220" height="90" viewBox="0 0 220 90" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* body */}
      <path d="M10 60 Q 20 40 60 38 L 130 34 Q 170 34 190 50 L 208 55 Q 214 60 208 66 L 20 66 Q 8 66 10 60 Z"
            fill="url(#body)" stroke="oklch(0.82 0.13 82)" strokeWidth="0.8" />
      {/* windows */}
      ...
      <circle cx="55" cy="70" r="6" fill="oklch(0.25 0 0)" stroke="oklch(0.82 0.13 82)" strokeWidth="0.6" />
      <circle cx="165" cy="70" r="12" fill="oklch(0.08 0 0)" />
      <circle cx="165" cy="70" r="6" fill="oklch(0.25 0 0)" stroke="oklch(0.82 0.13 82)" strokeWidth="0.6" />
      {/* tail light */}
