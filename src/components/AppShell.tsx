    ...
    <header className="sticky top-0 z-40 glass">
      <div className="mx-auto grid max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-4 py-3 sm:px-6">
        <Link to="/" className="flex min-w-0 items-center gap-2.5">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full gold-gradient text-background font-black shadow-glow">P</span>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="glass rounded-full p-0.5 flex text-xs">
            <button onClick={() => setLang("en")} className={`px-2.5 py-1 rounded-full transition ${lang === "en" ? "gold-gradient text-background font-semibold" : "text-muted-foreground"}`}>EN</button>
            <button onClick={() => setLang("hi")} className={`px-2.5 py-1 rounded-full transition ${lang === "hi" ? "gold-gradient text-background font-semibold" : "text-muted-foreground"}`}>हिं</button>
          </div>
        </div>
      </div>
    </header>
...
function NavLink({ to, children }: { to: string; children: ReactNode }) {
  return (
    <Link to={to} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition">
      {children}
    </Link>
  );
}
      ...
      <ul className="grid grid-cols-4 py-2">
        {[
          ...
          <li key={to} className="text-center">
            <Link to={to} className="flex flex-col items-center gap-1 text-[11px] text-muted-foreground [&.active]:text-primary" activeProps={{ className: "active" }}>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10 grid gap-8 sm:grid-cols-4">
        <div className="sm:col-span-2">
          <div className="flex items-center gap-2.5">
            <span className="grid h-8 w-8 place-items-center rounded-full gold-gradient text-background font-black">P</span>
            <span className="font-display text-xl gold-text">PellingCab</span>
          </div>
          <p className="mt-3 max-w-sm text-sm text-muted-foreground">
            Premium chauffeured travel across India. Half-day, full-day, city rides — booked in under a minute.
          </p>
        </div>
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            ...
            <li><Link to="/support" className="hover:text-primary">Contact</Link></li>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            ...
            <li><Link to="/history" className="hover:text-primary">My trips</Link></li>
            <li><Link to="/login" className="hover:text-primary">Sign in</Link></li>
          </ul>
        </div>
      </div>
      <p className="text-center text-xs text-muted-foreground pb-6">© {new Date().getFullYear()} PellingCab. All rights reserved.</p>
    </footer>
  );
}
